"""Connection lifecycle for Google Forms Connector."""
...
_SECRET = "google_forms_connections"
...
@chat.function(
    ...
    "Connect Google Forms account via credentials.",
    action_type="write",
    chain_callable=True,
...
async def connect_google_forms(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    ...
    record = {
        "id": cid,
        "label": params.label or "Google Forms Account",
...
@chat.function(
    "list_connections",
    "List connected Google Forms accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_google_forms(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
